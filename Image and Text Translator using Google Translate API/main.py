...
import pytesseract, os, platform, subprocess, time, easyocr, traceback, cv2
...
def period_wait():
    ...
    for z, x in enumerate(period):
        ...
        time.sleep(.2)
        if z <= period_len:
            z += 1; print(f"{yellow}{x * z}{reset}"); continue
        elif z == period_len:
            break
...
class TextTranslate():
    def __init__(self):
        ...
        self.translation = Translator.detect(self.inQuestion)
    ...
    def __enter__(self): print('enter method called'); return self
...
def translate_to_spanish(self):
    try:
        ...
        print(f'{red} ill fix you later--google{reset}'); pass
    except Exception as e:
...
def translate_to_french(self):
    try:
        ...
        print(f'{red} ill fix you later--google'); pass
    except Exception as e:
...
def translate_to_latin(self):
    try:
        ...
        print(f"{str(e)}, traceback.print_exc()"); pass
...
class ImageTranslate():
    ...
    def display_Originals(self):  ### Display images
        try:
            ...
            if display:
                time00 = time.time(); ctime00 = time.ctime(time00); print(f'[+] Display Opened at [{ctime00}]')
            else:
                print(f'[-] Viewer Could not Open Picture '); pass; cv2.imshow(self.img_loc); cv2.waitKey()
            if cv2.imshow('Result', result):
                print(f'[+] CV2 display opened [+]')
                key = cv2.waitKey(0) & 0xFF  # 1 ms delay
                if key == ord('q'): print(f'{red} [-] ** User Quit Key Hammered ** [-]')
            else:
